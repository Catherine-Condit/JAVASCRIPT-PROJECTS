...
function Input_Decimal(dot) {
    //This ensures the accidental clicking of the decimal point doesn't cause bugs in the operation.
    if (Calculator.Wait_Second_Operand === true) return; //if true, prevents adding the decimal point, since the calculator is in a state waiting for the second operand.
...
keys.addEventListener('click', (event) => {
    //the target variable is an oject that represents the element that was clicked.
    const { target } = event;
